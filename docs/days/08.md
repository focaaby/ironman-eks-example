...
根據 EKS [IAM roles for service accounts][https://docs.aws.amazon.com/eks/latest/userguide/iam-roles-for-service-accounts.html](1)，我們可以透過於在 Kubernetes 上的 Service Account 及 Pod annotation 設置 IAM role ARN 後，則 Pod 就可以取得此 IAM role 對應的權限。
...
近一步查看這個 Service Account Token 是什麼，其格式似乎是 [JSON Web Tokens（JWT）][https://jwt.io/](2) token 格式：
...
一般來說，若要使用 IRSA 功能，首先第一步驟是[建立 IAM OIDC Provider][https://docs.aws.amazon.com/eks/latest/userguide/enable-iam-roles-for-service-accounts.html](3) 於 EKS cluster 上。然而此步驟於先前定義 `eksctl` ClusterConfig 時則設定：
