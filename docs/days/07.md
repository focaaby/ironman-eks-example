...
除了 `eksctl` 之外，我們也可以透過 [CDK][https://docs.aws.amazon.com/cdk/v2/guide/getting_started.html](1) 方式來 provision EKS cluster 資源，其 CDK 也屬於 [Infrastructure as Code（IaC）][https://en.wikipedia.org/wiki/Infrastructure_as_code](2) 一種工具，可以更加方便使用者透過撰寫程式碼方式建構 AWS 架構。
...
接續，為了解此 Lambda Function 主要處理目的，我們可以在 CDK `@aws-cdk/aws-eks` 了解其架構：
...
換言之，此 Lambda function 也會依據其定義等候 EKS 資源被建立後才能視為完成。故有可能在此 Lambda function 也會花費較多一些時間來設定 EKS cluster。此部分我們也可以在原始碼上觀察到類似定義：
